...
class ArgumentsParser {
    ...
    static cLAToParamName(cLA) {
        if (cLA.toLowerCase() !== cLA) {
            throw new errors_1.BuidlerError(errors_1.ERRORS.ARGUMENTS.PARAM_NAME_INVALID_CASING, cLA);
        }
    ...
    parseBuidlerArguments(buidlerParamDefinitions, envVariableArguments, rawCLAs) {
        ...
        for (let i = 0; i < rawCLAs.length; i++) {
            ...
            if (taskName === undefined) {
                ...
                if (!this._isCLAParamName(arg, buidlerParamDefinitions)) {
                    throw new errors_1.BuidlerError(errors_1.ERRORS.ARGUMENTS.UNRECOGNIZED_COMMAND_LINE_ARG, arg);
                }
                i = this._parseArgumentAt(rawCLAs, i, buidlerParamDefinitions, buidlerArguments);
            }
            else {
                if (!this._isCLAParamName(arg, buidlerParamDefinitions)) {
        ...
        this._addBuidlerDefaultArguments(buidlerParamDefinitions, envVariableArguments, buidlerArguments);
        return {
    ...
    parseTaskArguments(taskDefinition, rawCLAs) {
        ...
        return Object.assign({}, paramArguments, positionalArguments);
    }
    _parseTaskParamArguments(taskDefinition, rawCLAs) {
        ...
        for (let i = 0; i < rawCLAs.length; i++) {
            ...
            if (!this._isCLAParamName(arg, taskDefinition.paramDefinitions)) {
                throw new errors_1.BuidlerError(errors_1.ERRORS.ARGUMENTS.UNRECOGNIZED_PARAM_NAME, arg);
            }
    ...
    _addBuidlerDefaultArguments(buidlerParamDefinitions, envVariableArguments, buidlerArguments) {
        for (const paramName of unsafe_1.unsafeObjectKeys(buidlerParamDefinitions)) {
        ...
        for (const paramName of Object.keys(taskDefinition.paramDefinitions)) {
            ...
            if (taskArguments[paramName] === undefined) {
                if (definition.isOptional) {
                    ...
                    throw new errors_1.BuidlerError(errors_1.ERRORS.ARGUMENTS.MISSING_TASK_ARGUMENT, paramName);
                }
            }
        }
    }
    ...
    _parseArgumentAt(rawCLAs, index, paramDefinitions, parsedArguments) {
        ...
        if (parsedArguments[paramName] !== undefined) {
            throw new errors_1.BuidlerError(errors_1.ERRORS.ARGUMENTS.REPEATED_PARAM, claArg);
        }
    ...
    _parsePositionalParamArgs(rawPositionalParamArgs, positionalParamDefinitions) {
        ...
        for (let i = 0; i < positionalParamDefinitions.length; i++) {
            ...
            if (rawArg === undefined) {
                if (!definition.isOptional) {
                    throw new errors_1.BuidlerError(errors_1.ERRORS.ARGUMENTS.MISSING_POSITIONAL_ARG, definition.name);
                }
        ...
        if (!hasVariadicParam &&
            ...
            throw new errors_1.BuidlerError(errors_1.ERRORS.ARGUMENTS.UNRECOGNIZED_POSITIONAL_ARG, rawPositionalParamArgs[positionalParamDefinitions.length]);
        }
        return args;
    }
}
