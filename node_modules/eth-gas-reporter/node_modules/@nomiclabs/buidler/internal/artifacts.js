...
function getArtifactFromContractOutput(contractName, contractOutput) {
    ...
    const bytecode = evmBytecode && evmBytecode.object ? evmBytecode.object : "";
...
async function saveArtifact(artifactsPath, artifact) {
    await fs_extra_1.default.writeJSON(artifactsPath + "/" + artifact.contractName + ".json", artifact, {
        spaces: 2
    });
}
...
async function readArtifact(artifactsPath, contractName) {
    ...
    if (!fs_extra_1.default.pathExistsSync(artifactPath)) {
        throw new errors_1.BuidlerError(errors_1.ERRORS.ARTIFACTS.NOT_FOUND, contractName);
    }
