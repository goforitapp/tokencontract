...
class Environment {
    ...
    constructor(config, buidlerArguments, tasks, extenders = []) {
        ...
        this.run = async (name, taskArguments = {}) => {
            ...
            if (taskDefinition === undefined) {
                throw new errors_1.BuidlerError(errors_1.ERRORS.ARGUMENTS.UNRECOGNIZED_TASK, name);
            }
            return this._runTaskDefinition(taskDefinition, taskArguments);
        };
        this.ethereum = lazy_1.lazyObject(() => construction_1.createProvider(buidlerArguments.network, config.networks));
    ...
    async _runTaskDefinition(taskDefinition, taskArguments) {
        ...
        if (taskDefinition instanceof task_definitions_1.OverriddenTaskDefinition) {
            runSuper = async (_taskArguments = taskArguments) => this._runTaskDefinition(taskDefinition.parentTaskDefinition, _taskArguments);
        }
        else {
            runSuper = async () => {
                throw new errors_1.BuidlerError(errors_1.ERRORS.TASK_DEFINITIONS.RUNSUPER_NOT_AVAILABLE, taskDefinition.name);
            };
        }
        const globalAsAny = global;
