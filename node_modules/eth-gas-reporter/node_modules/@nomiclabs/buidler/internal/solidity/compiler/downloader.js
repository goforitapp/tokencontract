...
class CompilerDownloader {
    ...
    async getCompilerBuild(version) {
        ...
        if (compilerBuild === undefined) {
            throw new errors_1.BuidlerError(errors_1.ERRORS.SOLC.INVALID_VERSION, version);
        }
    ...
    async downloadCompilersList() {
        try {
            ...
            throw new errors_1.BuidlerError(errors_1.ERRORS.SOLC.VERSION_LIST_DOWNLOAD_FAILED, error, this._localSolcVersion);
        }
    }
    async downloadCompiler(compilerBuild, downloadedFilePath) {
        ...
        try {
            ...
            throw new errors_1.BuidlerError(errors_1.ERRORS.SOLC.DOWNLOAD_FAILED, error, compilerBuild.version, this._localSolcVersion);
        }
    }
    async verifyCompiler(compilerBuild, downloadedFilePath) {
        ...
        const compilerKeccak256 = "0x" + ethereumjsUtil.keccak(compiler).toString("hex");
        if (expectedKeccak256 !== compilerKeccak256) {
            await fs_extra_1.default.unlink(downloadedFilePath);
            throw new errors_1.BuidlerError(errors_1.ERRORS.SOLC.INVALID_DOWNLOAD, compilerBuild.version, this._localSolcVersion);
        }
    }
