...
const buidler_params_1 = require("../params/buidler-params");
/**
...
class SimpleTaskDefinition {
     ...
     */
    constructor(name, isInternal = false) {
        this.name = name;
        ...
        this.action = () => {
            throw new errors_1.BuidlerError(errors_1.ERRORS.TASK_DEFINITIONS.ACTION_NOT_SET, name);
        };
    }
    get description() {
    ...
    addParam(name, description, defaultValue, type, isOptional = defaultValue !== undefined) {
        if (type === undefined) {
            if (defaultValue !== undefined && typeof defaultValue !== "string") {
                throw new errors_1.BuidlerError(errors_1.ERRORS.TASK_DEFINITIONS.DEFAULT_VALUE_WRONG_TYPE, name, this.name);
            }
    ...
    _validateNotAfterVariadicParam(name) {
        if (this._hasVariadicParam) {
            throw new errors_1.BuidlerError(errors_1.ERRORS.TASK_DEFINITIONS.PARAM_AFTER_VARIADIC, name, this.name);
        }
    }
    /**
    ...
    _validateNameNotUsed(name) {
        if (this._hasParamDefined(name)) {
            throw new errors_1.BuidlerError(errors_1.ERRORS.TASK_DEFINITIONS.PARAM_ALREADY_DEFINED, name, this.name);
        }
        if (Object.keys(buidler_params_1.BUIDLER_PARAM_DEFINITIONS).includes(name)) {
            throw new errors_1.BuidlerError(errors_1.ERRORS.TASK_DEFINITIONS.PARAM_CLASHES_WITH_BUIDLER_PARAM, name, this.name);
        }
    }
    /**
    ...
    _validateNoMandatoryParamAfterOptionalOnes(name, isOptional) {
        if (!isOptional && this._hasOptionalPositionalParam) {
            throw new errors_1.BuidlerError(errors_1.ERRORS.TASK_DEFINITIONS.MANDATORY_PARAM_AFTER_OPTIONAL, name, this.name);
        }
    }
    _validateParamNameCasing(name) {
        ...
        if (match === null) {
            throw new errors_1.BuidlerError(errors_1.ERRORS.TASK_DEFINITIONS.INVALID_PARAM_NAME_CASING, this.name, name);
        }
    }
    _validateNoDefaultValueForMandatoryParam(defaultValue, isOptional, name) {
        if (defaultValue !== undefined && !isOptional) {
            throw new errors_1.BuidlerError(errors_1.ERRORS.TASK_DEFINITIONS.DEFAULT_IN_MANDATORY_PARAM, name, this.name);
        }
    }
...
class OverriddenTaskDefinition {
    constructor(parentTaskDefinition, isInternal = false) {
     ...
     * Overriden tasks can't add new parameters.
     */
    addPositionalParam(name, description, defaultValue, type, isOptional) {
        return this._throwNoParamsOverrideError();
    }
    /**
     * Overriden tasks can't add new parameters.
     ...
     * Overriden tasks can't add new parameters.
     */
    addVariadicPositionalParam(name, description, defaultValue, type, isOptional) {
        return this._throwNoParamsOverrideError();
    }
    /**
     * Overriden tasks can't add new parameters.
    ...
    _throwNoParamsOverrideError() {
        throw new errors_1.BuidlerError(errors_1.ERRORS.TASK_DEFINITIONS.OVERRIDE_NO_PARAMS, this.name);
    }
}
